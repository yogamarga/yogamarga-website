...
class Jekyll::Thumbnail < Liquid::Tag
  ...
  def render(context)
    if @source
      ...
      if !Dir.exists?(Dir.pwd + "#{File.dirname(source)}/tn")
        Dir.mkdir(Dir.pwd + "#{File.dirname(source)}/tn")
      end

      dest = "#{File.dirname(source)}/tn/#{File.basename(source, ext)}_#{desc}#{ext}"
      dest_path = Dir.pwd + "#{dest}"
